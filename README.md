# 📘 Tabela de Desafios de Python — com Status e Dificuldade

## 🟦 Categorias: Lógica, Funções, Arquivos (iniciante → intermediário)

| Nº | Desafio | Dificuldade | Status |
|---:|---------|-------------|--------|
| 1 | Agenda Simples (contatos em lista) | ⭐ Fácil |  **Concluído** ✔ |
| 2 | Sistema de Login com arquivo .txt | ⭐⭐ Médio | Em espera |
| 3 | Pedra, Papel e Tesoura (placar + rounds) | ⭐ Fácil | Em espera |
| 4 | Organizador de Arquivos (os + shutil) | ⭐⭐⭐ Difícil | Em espera |
| 5 | Gerador de Senhas configurável | ⭐⭐ Médio | Em espera |
| 6 | Conversor de Moedas usando API externa | ⭐⭐⭐ Difícil | Em espera |
| 7 | Lista de Tarefas com prioridade + arquivo | ⭐⭐ Médio | Em espera |
| 8 | Sistema de Gerenciamento de Tarefas com Prioridade | ⭐⭐ Médio | Em espera |
| 9 | Agenda de Contatos (JSON) | ⭐⭐ Médio | Em espera |
| 10 | Sistema de Login com Hash (bcrypt/sha256) | ⭐⭐⭐ Difícil | Em espera |
| 11 | Calculadora Científica (histórico) | ⭐⭐ Médio | Em espera |
| 12 | Organizador de Arquivos (versão 2 aprimorada) | ⭐⭐⭐ Difícil | Em espera |
| 13 | Conversor de Arquivos (txt ↔ json ↔ csv) | ⭐⭐⭐ Difícil | Em espera |
| 14 | Ferramenta de Backup ZIP | ⭐⭐ Médio | Em espera |
| 15 | Web Scraper (notícias em CSV) | ⭐⭐⭐ Difícil | Em espera |
| 16 | Gerenciador de Senhas Criptografadas | ⭐⭐⭐ Difícil | Em espera |

---

## 🟩 Categorias: Programação Orientada a Objetos (POO)

| Nº | Desafio | Dificuldade | Status |
|---:|---------|-------------|--------|
| 17 | Classe Conta Bancária Completa | ⭐⭐ Médio | Em espera |
| 18 | Sistema de Biblioteca (objetos + relacionamentos) | ⭐⭐⭐ Difícil | Em espera |
| 19 | Sistema de RPG (POO + combate) | ⭐⭐⭐ Difícil | Em espera |
| 20 | RPG por Terminal (classes + inventário + turnos) | ⭐⭐⭐ Difícil | Em espera |

---

## 🟥 Categorias: Avançado — Terminal, Rede, APIs, Automação

| Nº | Desafio | Dificuldade | Status |
|---:|---------|-------------|--------|
| 21 | Chat de Terminal (Sockets) | ⭐⭐⭐ Difícil | Em espera |
| 22 | Conversor de Vídeo usando FFmpeg | ⭐⭐⭐ Difícil | Em espera |
| 23 | Gerador de QR Code | ⭐ Fácil | Em espera |
| 24 | Editor de Lista de Compras Avançado | ⭐⭐ Médio | **Concluído** ✔ |
| 25 | Mini-Sistema de Votação | ⭐⭐ Médio | Em espera |
| 26 | API Fake (Flask ou FastAPI — CRUD completo) | ⭐⭐⭐ Difícil | Em espera |
| 27 | Jogo Snake no Terminal (curses) | ⭐⭐⭐ Difícil | Em espera |
| 28 | Mini Banco (transferências + extrato) | ⭐⭐⭐ Difícil | Em espera |
| 29 | Bot de Automação (PyAutoGUI) | ⭐⭐ Médio | Em espera |

